    ...
    <div id="main">
        <div class="row m-0 pb-2 d-flex justify-content-end" id="search-sort">
            ...
            <div class="col-3 pl-0">
                <div content="Search information User" v-tippy class="input-group">
            ...
            <div class="pr-1">
                <div class="input-group ">
                    <button content="Add Account User" v-tippy data-toggle="modal" data-target="#addUser" type="button"
            ...
            <table class="table table-bordered">
                <thead>
                    <tr>
                        ...
                        <th scope="col"><i class="fa-brands fa-line"></i>Username</th>
                    ...
                    <tr v-for="(user, index) in users" :key="index">
                        ...
                        <td class="table-cell name">
                            <div class="nameAvatar">
                                <img :src="user.avatar ? user.avatar : require('@/assets/avatar.jpg')" alt="">
                                <span class="nameUser">{{ user.name }}</span>
                            </div>
                        </td>
                        <td class="table-cell displaytext break">{{ user.email }}</td>
                        <td class="table-cell displaytext break">{{ user.username }}</td>
                        <td class="table-cell displayphone break text-center">{{ user.phone ? user.phone : 'N/A'
                            }}
                        </td>
                        <td class="table-cell displaytext">{{ user.address ? truncatedTitle(user.address) : 'N/A'
                            }}
                        </td>
...
export default {
    name: "ManageUser",
    setup() {
        document.title = "Manage Account User| Blog"
    },
    components: {
        paginate: Paginate,
        TableLoading,
    ...
    mounted() {
        emitEvent('eventTitleHeader', 'Manage Account');
        ...
        onEvent('eventUpdateIsBlock', (id_user) => {
            this.users.forEach(user => {
                if (user.id == id_user) {
                    if (user.is_block == 1) user.is_block = 0;
                    else user.is_block = 1;
                }
            });
        });
    ...
    methods: {
        ...
        getUsers: async function () {
            ...
            this.query = '?search=' + this.search + '&typesort=' + this.big_search.typesort + '&sortlatest=' + this.big_search.sortlatest
                 + '&is_block=' + this.big_search.is_block + '&paginate=' + this.big_search.perPage + '&page=' + this.big_search.page;
            window.history.pushState({}, null, this.query);

            try {
                const { data } = await AdminRequest.get('admin/users' + this.query)
        ...
        selectUser: function (userSelected) {
        ...
        handleSelect: function (userId) {
            ...
            if (index === -1) this.selectedUsers.push(userId);
...
.blockUser .fa-lock:hover {
    transition: all 0.5s ease;
    color: red;
}

.blockUser .fa-lock-open:hover {
    transition: all 0.5s ease;
    color: green;
}
